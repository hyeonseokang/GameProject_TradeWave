...
#define CEMETERY 40000
#define CINTAMANI 30000
#define WRISTBAND 50000
void Buy::TradeInit(LayerColor * layer, int * price, int * have, int * Money, Inventory *_in)
{
	inven = _in;
	money = Money; //돈 저장 
	bprice = price; // 시세 저장
	...
	auto Touch = MenuItemImage::create("transparency.png",
		"transparency.png", CC_CALLBACK_1(Buy::Final, this));
	...
	ScrollLayer = LayerColor::create(Color4B(35, 255, 23, 0), 600, 625);
	...
	///시크릿 교역품////
	secret = RandomHelper::random_int(1, 1);
	if (secret == 1)
	{
		secretbool = true;
		SecretButton = MenuItemImage::create("Trade/ItemButton.png", "Trade/ItemButton.png", CC_CALLBACK_1(Buy::MaterialMenu, this));
		...
		secret = RandomHelper::random_int(2, 4);
		...
		if (secret == 2)
		{
			...
			SecretName->setPosition(Vec2(230, -25 + 125));
			SecretName->setColor(Color3B(0, 0, 0));
			SecretPrice = Label::create("$40,000", "fonts/JejuGothic-Regular.ttf", 30);
		...
		if (secret == 3)
		{
			...
			SecretName->setPosition(Vec2(230, -25 + 125));
			SecretName->setColor(Color3B(0, 0, 0));
			SecretPrice = Label::create("$30,000", "fonts/JejuGothic-Regular.ttf", 30);
			SecretPrice->setPosition(Vec2(340, 405 - 500 + 125));
			SecretPrice->setColor(Color3B(0, 0, 0));
			ScrollLayer->addChild(Material);
			ScrollLayer->addChild(SecretName);
		...
		if (secret == 4)
		{
			...
			SecretName->setPosition(Vec2(230, -25 + 125));
			SecretName->setColor(Color3B(0, 0, 0));
			SecretPrice = Label::create("$50,000", "fonts/JejuGothic-Regular.ttf", 30);
			SecretPrice->setPosition(Vec2(340, 405 - 500 + 125));
			SecretPrice->setColor(Color3B(0, 0, 0));
			ScrollLayer->addChild(Material);
			ScrollLayer->addChild(SecretName);
	...
	{
		ADDVec2 = 0;
		ScrollLayer->setContentSize(Size(600, 400));
	}
	for (int a = 4; a < 24; a++)
	{
	...
	FinalClose = MenuItemImage::create("All/CloseButton00.png",  "All/CloseButton01.png", CC_CALLBACK_1(Buy::Final,this));
	FinalClose->setPosition(Vec2(990, 640));
	FinalClose->setTag(1);

	CountSprite = Sprite::create("Trade/Buy/Count00.png");
	...
	Next = MenuItemImage::create("Trade/Buy/arrow00.png", "Trade/Buy/arrow01.png", CC_CALLBACK_1(Buy::Final, this));
	Next->setPosition(Vec2(850, 313));
	Next->setScale(1.5);
	Next->setTag(2);
	Back = MenuItemImage::create("Trade/Buy/arrow00.png", "Trade/Buy/arrow01.png", CC_CALLBACK_1(Buy::Final, this));
	Back->setPosition(Vec2(430, 313));
	Back->setRotationY(180);
	Back->setScale(1.5);
	Back->setTag(3);
	BSbutton = MenuItemImage::create("Trade/Buy/BuyButton00.png", "Trade/Buy/BuyButton01.png", CC_CALLBACK_1(Buy::Final, this));
	...
	("%d(개)", Count), "fonts/JejuGothic-Regular.ttf", 45);
	CountLabel->setColor(Color3B(255, 255, 255));
	CountLabel->setPosition(Vec2(640, 313));
...
void Buy::FinalBS(int _Material, int _Count)
{
	CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("BGM/trade_upgrade.mp3");
	if (_Material != 29)
	{
		UserDefault::getInstance()->setIntegerForKey("MONEY", 
			UserDefault::getInstance()->getIntegerForKey("MONEY") - bprice[_Material] * _Count);
	...
	else
	{
		if (secret == 2)//묘지
		{
			UserDefault::getInstance()->setIntegerForKey("MONEY", *money - CEMETERY * _Count);
			*money = *money - CEMETERY * _Count;
			UserDefault::getInstance()->setIntegerForKey("HCEMETERY", _Count);
			secretbool = true;
		}
		if (secret == 3)//여의주
		{
			UserDefault::getInstance()->setIntegerForKey("MONEY", *money - CINTAMANI * _Count);
			*money = *money - CINTAMANI * _Count;
			UserDefault::getInstance()->setIntegerForKey("HCINTAMANI", _Count);
			secretbool = true;
		}
		if (secret == 4)//팔찌
		{
			UserDefault::getInstance()->setIntegerForKey("MONEY", *money - WRISTBAND * _Count);
			*money = *money - WRISTBAND * _Count;
			UserDefault::getInstance()->setIntegerForKey("HWRISTBAND", _Count);
			secretbool = true;
		}
	}
	if (secretChoice == 0)
		UserDefault::getInstance()->setIntegerForKey("HMAX",
			UserDefault::getInstance()->getIntegerForKey("HMAX") + _Count);
	FinalLayer->setVisible(false);
	inven->ChangeLabel(_Material);
	MAXbar->setScale(1.0);
}

void Buy::Final(Ref * pSender)
{
		...
		case 2:
			...
			else if(UserDefault::getInstance()->getIntegerForKey("MONEY") > (bprice[Materialcount] * (Count + 1))&& UserDefault::getInstance()->getIntegerForKey("HMAX") + Count+1<=UserDefault::getInstance()->getIntegerForKey("MAX"))
			{
				++Count;
		...
		if (secret == 2&& secretChoice ==1)
			MoneyCountLabel->setString(StringUtils::format("$ %d", Count * CEMETERY));
		else if (secret == 3 && secretChoice == 1)
			MoneyCountLabel->setString(StringUtils::format("$ %d", Count * CINTAMANI));
		else if (secret == 4 && secretChoice == 1)
			MoneyCountLabel->setString(StringUtils::format("$ %d", Count * WRISTBAND));
		
	
	CountLabel->setString(StringUtils::format("%d(개)",Count));
}

void Buy::MaterialMenu(Ref * pSender)
{
	secretChoice = 0;
	char *name;
	switch (((Node*)pSender)->getTag())
	...
	MaterialLabel->setString(StringUtils::format("%s 구매하시겠습니까?", name));
	CountLabel->setString("0(개)");
	MoneyCountLabel->setString("$ 0");
	FinalLayer->setVisible(true);
}

void Buy::FarmingMenu(LayerColor *layer)
{
	for (int a = 0; a < 4; a++)
	{
		MaterialButton[a+4] = MenuItemImage::create("Trade/ItemButton.png", "Trade/ItemButton.png", CC_CALLBACK_1(Buy::MaterialMenu, this));
	...
	for (int a = 0; a < 4; a++)
	{
		...
		PriceMaterial[a+4]->setPosition(Vec2(230, 405 - a * 125 + ADDVec2));
	...
	for (int a = 0; a < 4; a++)
	{
		...
		LloadPrice[a+4]->setPosition(Vec2(400, 405 - a * 125 + ADDVec2));
		LloadPrice[a+4]->setColor(Color3B(0, 0, 0));
		layer->addChild(LloadPrice[a+4], 1);
	}
}

void Buy::SeafoodMenu(LayerColor *layer)
{
	for (int a = 0; a < 4; a++)
	{
		MaterialButton[a+8] = MenuItemImage::create("Trade/ItemButton.png", "Trade/ItemButton.png", CC_CALLBACK_1(Buy::MaterialMenu, this));
	...
	for (int a = 0; a < 4; a++)
	{
		...
		PriceMaterial[a + 8]->setPosition(Vec2(230, 410 - a * 125 + ADDVec2));
...
void Buy::JewelryMenu(LayerColor *layer)
{

	for (int a = 0; a < 4; a++)
	{
		MaterialButton[a + 12] = MenuItemImage::create("Trade/ItemButton.png", "Trade/ItemButton.png", CC_CALLBACK_1(Buy::MaterialMenu, this));
	...
	for (int a = 0; a < 4; a++)
	{
		...
		//PriceMaterial[a]->setPosition(Vec2(340, 405-a*125 + ADDVec2));
		PriceMaterial[a + 12]->setPosition(Vec2(230, 410 - a * 125 + ADDVec2));
...
void Buy::MineralMenu(LayerColor *layer)
{
	for (int a = 0; a < 4; a++)
	{
		MaterialButton[a + 16] = MenuItemImage::create("Trade/ItemButton.png", "Trade/ItemButton.png", CC_CALLBACK_1(Buy::MaterialMenu, this));
	...
	for (int a = 0; a < 4; a++)
	{
		...
		//PriceMaterial[a]->setPosition(Vec2(340, 405-a*125 + ADDVec2));
		PriceMaterial[a + 16]->setPosition(Vec2(230, 410 - a * 125 + ADDVec2));
...
void Buy::ClothMenu(LayerColor *layer)
{
	for (int a = 0; a < 4; a++)
	{
		MaterialButton[a + 20] = MenuItemImage::create("Trade/ItemButton.png", "Trade/ItemButton.png", CC_CALLBACK_1(Buy::MaterialMenu, this));
	...
	for (int a = 0; a < 4; a++)
	{
		...
		//PriceMaterial[a]->setPosition(Vec2(340, 405-a*125 + ADDVec2));
		PriceMaterial[a + 20]->setPosition(Vec2(230, 410 - a * 125 + ADDVec2));
